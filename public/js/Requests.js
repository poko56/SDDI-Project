  ...
  try {
    ...
    <div class="card">
      <div style="padding:.875rem 1.25rem;border-bottom:1px solid var(--wire);display:flex;flex-wrap:wrap;gap:.625rem;align-items:flex-end">
        ...
        <div class="fg" style="flex:1 1 120px;margin:0"><label class="fl">ประเภท</label><select class="fc" id="f-st" onchange="applyF()"><option value="">ทั้งหมด</option>${['ไฟฟ้า','ประปา','โครงสร้าง','อุปกรณ์อิเล็กทรอนิกส์','เครื่องปรับอากาศ'].map(s=>`<option${flt.category===s?' selected':''}>${s}</option>`).join('')}</select></div>
            ...
            <div class="tw"><table>
              ...
              <tbody>
                ${r.materials_used.map(m=>`<tr>
                  ...
                  <td class="text-xs text-muted tr">${fmtDate(m.withdrawn_at,true)}</td>
