...
const DB_PATH = isFirebase ? path.join('/tmp', 'maintenance.db') : path.join(__dirname, 'maintenance.db');
let db = null;
...
import { getFirestore, collection, doc, getDoc, getDocs, addDoc, updateDoc, deleteDoc, setDoc, query, where, orderBy, limit, onSnapshot, serverTimestamp, increment, Timestamp, writeBatch, getCountFromServer } from 'https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js';
...
async function getDb() {
  ...
  if (fs.existsSync(DB_PATH)) {
    ...
    await seedData();
    saveDb();
>>>>>>> c3043c2b8db97368e9f89ee577c64a87e983f8f5
  }
  return db;
}
